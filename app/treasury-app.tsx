...
function DepositModal({ close, submit }: { close: () => void; submit: (amount: string) => void }) {
  ...
  return <ModalShell close={close}><form onSubmit={(event) => { event.preventDefault(); void submit(amount); }}><ModalHeader eyebrow="Native XLM · Stellar Asset Contract" title="Nạp tài sản vào treasury" close={close}/><label>Số lượng XLM<input required min="0.0000001" step="0.0000001" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Ví dụ: 9000" /><small className="input-hint">Đây là tiền chuyển từ ví Freighter vào treasury, không phải nút cấp thêm tiền cho ví.</small></label><div className="modal-rule"><ShieldCheck /><span>Friendbot Testnet thường cấp 10.000 XLM cho một ví. Cần giữ lại ít nhất 1 XLM, nên hãy nạp tối đa khoảng 9.999 XLM từ mỗi ví. Freighter sẽ hiện transaction trước khi bạn ký.</span></div><button className="modal-submit">Mở Freighter để xác nhận <ArrowRight /></button></form></ModalShell>;
}
